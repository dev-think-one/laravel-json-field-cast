...
class FileJsonTest extends TestCase
{
    ...
    {
        $this->assertNull($this->user->getRawOriginal('json_file'));
        $this->assertInstanceOf(\JsonFieldCast\Json\FileJsonField::class, $this->user->json_file);
        /** @var \JsonFieldCast\Json\FileJsonField::class $file */
        $file = $this->user->json_file;
    ...
    {
        $this->user->json_file->storeUploadedFile(UploadedFile::fake()->image('My Avatar.jpg'));
        $this->user->save();
        $this->user->refresh();

        $this->assertTrue($this->user->json_file->exists());
        $this->assertEquals('my-avatar.jpg', $this->user->json_file->fileName());
        $this->assertEquals(storage_path('app/my-avatar.jpg'), $this->user->json_file->filePath());
        $this->assertEquals('/storage/my-avatar.jpg', $this->user->json_file->fileUrl());
        $this->assertInstanceOf(StreamedResponse::class, $this->user->json_file->fileDownload());
    }
    ...
    public function cast_delete_file()
    {
        ...
        $filePath = $this->user->json_file->filePath();
        $this->assertTrue(file_exists($filePath));
        $this->assertTrue($this->user->json_file->exists());
        $this->user->json_file->delete();
        $this->assertFalse(file_exists($filePath));
        $this->assertFalse($this->user->json_file->exists());
    }
}
