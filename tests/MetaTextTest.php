...
class MetaTextTest extends TestCase
{
    ...
    public function method_increment_return_error_if_not_numeric()
    {
        ...
        $this->user2->text_meta->increment('position');
    }
    ...
    {
        $this->expectException(\InvalidArgumentException::class);
        $this->user2->text_meta->decrement('position');
    }
    ...
    public function to_morph()
    {
        ...
        $this->user2->text_meta->toMorph('user', $fakeUser);
        $this->user2->save();
        Arr::get($this->user2->text_meta->getRawData(), 'user.id', $fakeUser->getKey());
        Arr::get($this->user2->text_meta->getRawData(), 'user.class', $fakeUser->getMorphClass());

        $this->user2->text_meta->toMorph('user1', $fakeUser, 'idFoo', 'classBar');
        $this->user2->save();
        Arr::get($this->user2->text_meta->getRawData(), 'user1.idFoo', $fakeUser->getKey());
        Arr::get($this->user2->text_meta->getRawData(), 'user1.classBar', $fakeUser->getMorphClass());

        $this->user2->text_meta->toMorph('', $fakeUser);
        $this->user2->save();
        Arr::get($this->user2->text_meta->getRawData(), 'id', $fakeUser->getKey());
        Arr::get($this->user2->text_meta->getRawData(), 'class', $fakeUser->getMorphClass());
    }
    ...
    public function from_morph()
    {
        ...
        $user = $this->user2->text_meta->fromMorph('user');
        $this->assertInstanceOf(User::class, $user);
        $this->assertEquals($fakeUser->getKey(), $user->getKey());

        $user = $this->user2->text_meta->fromMorph('user1');
        $this->assertNull($user);

        $user = $this->user2->text_meta->fromMorph('user1', null, 'idFoo', 'classBar');
        $this->assertInstanceOf(User::class, $user);
        $this->assertEquals($fakeUser->getKey(), $user->getKey());

        $user = $this->user2->text_meta->fromMorph('');
        $this->assertInstanceOf(User::class, $user);
        $this->assertEquals($fakeUser->getKey(), $user->getKey());
    }
    ...
    public function from_morph_returns_defaultValue()
    {
        ...
        $user = $this->user2->text_meta->fromMorph('fake_user', $defaultUser);
        $this->assertInstanceOf(User::class, $user);
        $this->assertEquals($defaultUser->getKey(), $user->getKey());

        $user = $this->user2->text_meta->fromMorph('user_wrong_class', $defaultUser);
        $this->assertInstanceOf(User::class, $user);
        $this->assertEquals($defaultUser->getKey(), $user->getKey());

        $user = $this->user2->text_meta->fromMorph('user_wrong_id', $defaultUser);
        $this->assertInstanceOf(User::class, $user);
        $this->assertEquals($defaultUser->getKey(), $user->getKey());
    }
}
